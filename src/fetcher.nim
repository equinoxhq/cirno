...
proc fetchFflagList*(): Option[string] =
  try:
    let request = curl.get(
      "https://github.com/MaximumADHD/Roblox-FFlag-Tracker/raw/refs/heads/main/AndroidApp.json"
    )
    if request.code != 200:
    ...
    info "Fetched FFlag list successfully.", size = request.body.len
    return some(request.body)
  except CatchableError as exc:
    error "Failed to send request to Github", err = exc.msg
